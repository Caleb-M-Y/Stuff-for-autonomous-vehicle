from base_motor_cont import BaseMotor
...
if __name__ == "__main__":  # Test only the encoder part
    ...
    # SETUP
    #em = EncodedMotor(driver_ids=(16, 17, 18), encoder_ids=(27, 26))  # right motor, encoder's green and yellow on GP19 and GP20
    em = EncodedMotor((21, 20, 19), (7, 6))  # left motor, encoder's green and yellow on GP10 and GP11
    ...
    for i in range(100):
        em.backward((i + 1) / 100)
        print(f"b, dc: {i}%, enc_cnt: {em.encoder_counts}")
        sleep(4 / 100)  # 4 seconds to ramp up
    for i in reversed(range(100)):
        em.backward((i + 1) / 100)
        print(f"b, dc: {i}%, enc_cnt: {em.encoder_counts}")
